...
using set = std::unordered_set<int_pair, collections::TupleHash>;
...
int dp(const strings& grid, int row, int col, int current, set& memo)
{
    // negative base case
    if (memo.contains(std::make_pair(row, col)))
        return 0;
...
int Compute(const strings& grid)
{
    ...
    for (auto head : trailheads) {
        set memo {};
        score += dp(grid, std::get<0>(head), std::get<1>(head), 0, memo);
    }
